...
function init() {
  ...
  agentRole.textContent = buildingData.agent.role;
  agentName.textContent = buildingData.agent.name;
  agentPhone.textContent = buildingData.agent.phone;
  agentPhone.href = buildWhatsAppUrl({ unit: "general", floor: "general", areaM2: null });
  agentPhone.target = "_blank";
  agentPhone.rel = "noreferrer";
  agentEmail.textContent = buildingData.agent.email;
  agentEmail.href = `mailto:${buildingData.agent.email}`;
  renderBuilding();
  renderFloorSelector();
...
function render() {
  ...
  if (!selectedUnit) {
    ...
    axoImage.src = buildingData.building.groundFloorAsset || "./assets/floors/floor_placeholder.svg";
...
function buildWhatsAppUrl(unit) {
  const raw = buildingData.agent.whatsapp.replace(/\D/g, "");
  const text = `Hola Alan, quiero consultar por la unidad ${unit.unit} del piso ${unit.floor}, de ${unit.areaM2 || "superficie a definir"} m2 en ${buildingData.building.name}.`;
...
function savePendingAdminChanges() {
  state.unitOverrides = {
    ...state.unitOverrides,
    ...state.pendingUnitOverrides
  };
  ...
  render();
  renderSelectedAdminFloorUnits();
  updateAdminSaveState(false, "Cambios guardados.");
}
